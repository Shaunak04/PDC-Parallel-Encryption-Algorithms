...
int main(){
	...
	string decrypted;
	vector<string> deciphertext;
	string text[deciphertext.size()];
	...
	int k=0;
	for(k=0;k<deciphertext.size()-1;k++){	
		writeObj2 << deciphertext[k];
	}
	writeObj2<<deciphertext[k];
	writeObj2.close(); 
	cout << "Decryption file creation Completed."<<endl;
	cout <<endl<<"Time taken is:"<<(t2-t1)<<"s"<<endl;  
}
